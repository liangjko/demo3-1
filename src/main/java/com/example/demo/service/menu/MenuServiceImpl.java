...
public class MenuServiceImpl implements MenuService {
	...
	@Override
	public List<MenuVo> getLeftMenu(String code) {
		List<MenuVo> list = menuMapper.getLeftMenu(code);
		List<MenuVo> roots = new ArrayList<>();
		for (int i = 0; i < list.size(); i++) {
			for (int j = 0; j < list.size(); j++) {
				if (StringUtils.hasLength(list.get(j).getFaIndex())) {
					if (list.get(j).getFaIndex().equals(list.get(i).getIndex())) {
						List<MenuVo> child = new ArrayList<>();
						if (list.get(i).getChildList() != null) {
							child.addAll(list.get(i).getChildList());
						}
						child.add(list.get(j));
						list.get(i).setChildList(child);
					}
				}
			}
		}
		list.forEach(fa->{
			if(!StringUtils.hasLength(fa.getFaIndex())){
				roots.add(fa);
			}
		});
		return roots;
	...
	@Override
	public PageValue getPageValue(String tagId) {
		PageValue value = menuMapper.getPageValue(tagId);
		return value;
	}
...
//			PageInfo<Good> pageInfo = new PageInfo(menuMapper.getGoodsList());
//
//			result.put("code", "0");
//			result.put("msg", "操作成功！");
...
//			result.put("count", pageInfo.getTotal());
	...
	public Size selectSize() {
		return menuMapper.selectSize();
	}
	
	@Override
	public void updateSize(Size size) {
		menuMapper.updateSize(size);
	}
	...
	public void getKill(String code) {
		List<History> list = menuMapper.findHistory(code);
